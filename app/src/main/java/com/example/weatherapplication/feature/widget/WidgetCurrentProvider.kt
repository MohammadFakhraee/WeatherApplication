...
class WidgetCurrentProvider : AppWidgetProvider() {
                ...
                appWidgetIds.forEach { appWidgetId ->
                    val remoteViews: RemoteViews =
                        when (weatherCurrentUiState) {
                            is WidgetWeatherCurrentUiState.DataStateWidget -> onDataLoaded(context, weatherCurrentUiState.widgetWeatherCurrentUi)
                            is WidgetWeatherCurrentUiState.ErrorStateWidget -> onErrorState(context, weatherCurrentUiState.messageId)
                        }
    ...
    private fun onDataLoaded(context: Context, widgetWeatherCurrentUi: WidgetWeatherCurrentUi): RemoteViews {
        return RemoteViews(context.packageName, R.layout.widget_weather_current).apply {
            ...
            setTextViewText(R.id.currentTempTv, context.getString(R.string.celsius_degree, widgetWeatherCurrentUi.currentTempC))
            setTextViewText(R.id.locationTv, widgetWeatherCurrentUi.cityName)
            ImageLoader.loadBitmap(context, widgetWeatherCurrentUi.iconUrl) { bitmap ->
